...
def run_acic(data_base_dir ,output_dir='../../dragonnet/',
             knob_loss=dragonnet_loss_binarycross,
             ratio=1., dragon='', folder='scaling'):
    
    print("************************************** the output directory is: ", output_dir)
    covariate_csv = os.path.join(data_base_dir, 'data', 'x.csv')
    x_raw = load_and_format_covariates(covariate_csv)
    simulation_dir = os.path.join(data_base_dir,'data', 'practice_scaling', folder)
...
def turn_knob(knob='dragonnet', folder='a'):
    base_path = os.getcwd()
    output_dir = os.path.join(base_path, 'data',  knob)

    if knob == 'dragonnet':
        run_acic(data_base_dir=base_path, output_dir=output_dir, folder=folder,
...
def main():
    ...
    parser.add_argument('--folder', type=str, default='scaling', help='which datasub directory')

    args = parser.parse_args()

    turn_knob(args.knob, args.folder)
